...
\subsection{グラフによる記法}
グラフによるマルコフジャンプ過程の記述をまとめておく。\\
\textbf{記号}
\begin{itemize}
    \item 頂点: $n \in \{1,2,\cdots,N\}$
    \item 有向辺: $e \in \{1,2,\cdots,E\}$
...
\begin{align}
    \mathbb{J}(\pi) &= 0
\end{align}
なる平衡状態$\pi$が存在するとする。このとき、
\begin{align}
    \mathbb{F}(\pi) &= \mathcal{M}_{\Gamma^{\pm}}^{-1}(\mathbb{J}(\pi)) = 0
\end{align}
であるから、
\begin{align}
    \mathbb{F}(\rho) &= \mathbb{F}(\rho) - \mathbb{F}(\pi)\\
    &= \{k_{\mathrm{B}} [L_{\mu}, \ln \rho - \ln \pi]\}_{\mu \in \mathcal{L}} \\
    &= -\{[\phi, L_{\mu}]\}_{\mu \in \mathcal{L}} \\
    &= - \nabla_{\mathbb{L}} \phi
\end{align}
となる$\phi = -k_{\mathrm{B}} (\ln \rho - \ln \pi)$が存在する。
...
Gradient & $\nabla$ & $\nabla_{L}$ \\
\hline
Continuity & $dp/dt = \nabla^{\top}J(p)$ & $\mathcal{D}(\rho) = \nabla_{L}^{*}\mathbb{J}(\rho)$ \\
\hline
Current & $K_{e} = K_{e}^{+} - K_{e}^{-}$ & $\mathbb{J}(\rho) = [L, \, \Gamma \otimes \rho]$ \\
\hline
Force & $F_{e} = \ln(K_{e}^{+}/K_{e}^{-})$ & $\mathbb{F}(\rho) = [L, \ln(\Gamma \otimes \rho)]$ \\
\hline
