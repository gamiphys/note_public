...
% \end{align}
% が成り立つ(ここで、時間平均がアンサンブル平均に置き換わる、エルゴードの定理を用いている)。\footnote{メモ:ここの計算の詳細はiPadのメモ機能のところにスクショがある。}\\
...
% \begin{align}
%     1=\ev{e^{-\hat{\sigma}}}&=1-\ev{\epsilon \int_{0}^{\tau} \dd t \hat{J}(t) \sin(\omega t)} + \frac{1}{2}\ev{\epsilon^2 \int_{0}^{\tau} \dd t' \int_{0}^{\tau} \dd t \hat{J}(t')\hat{J}(t) \sin(\omega t')\sin(\omega t)} + O(\epsilon^3)\\
% \end{align}
% を得る。ここで、
% \begin{equation}
